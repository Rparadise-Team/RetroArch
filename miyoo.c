...
void miyoo_menu_open_native_quickmenu(void)
{
    ...
    bool menu_alive          = menu_st && (menu_st->flags & MENU_ST_FLAG_ALIVE);
    ...
    /* Prevent stale/held OK input from being consumed by the
     * first native quick menu entry on transition frame(s). */
    if (menu_st)
