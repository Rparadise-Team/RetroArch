...
static void *sdl_miyoomini_gfx_init(const video_info_t *video,
...
#ifdef HAVE_SDL2
   SDL_SetHint(SDL_HINT_RENDER_DRIVER, "Miyoo Mini");
#endif
   ...
   if (SDL_RenderSetLogicalSize(vid->renderer,
            SDL_MIYOOMINI_WIDTH, SDL_MIYOOMINI_HEIGHT) < 0)
   ...
   vid->overlay_rect.w    = SDL_MIYOOMINI_WIDTH;
   vid->overlay_rect.h    = SDL_MIYOOMINI_HEIGHT;
#endif
   ...
   if (vid->renderer &&
         !sdl_miyoomini_ensure_texture(vid, &vid->menu_tex,
               SDL_MIYOOMINI_WIDTH, SDL_MIYOOMINI_HEIGHT, false))
      goto error;
...
static bool sdl_miyoomini_gfx_frame(void *data, const void *frame,
   ...
   if (likely(!vid->menu_active)) {
      ...
      /* Upload to SDL texture and present via Miyoo renderer */
      if (!sdl_miyoomini_ensure_texture(vid, &vid->frame_tex,
...
static bool sdl_miyoomini_overlay_load(void *data, const void *image_data, unsigned num_images) {
	...
	if (!vid) return false;
        ...
        SDL_Surface *texture_surface = cpu_surface;
        SDL_Surface *rotated_surface = NULL;

        SDL_SetSurfaceBlendMode(cpu_surface, SDL_BLENDMODE_BLEND);
        SDL_SetSurfaceAlphaMod(cpu_surface, alpha);
        ...
        if (rotate != E_MI_GFX_ROTATE_0)
        {
           rotated_surface = SDL_CreateRGBSurfaceWithFormat(0,
                 (rotate & 1) ? cpu_surface->h : cpu_surface->w,
                 (rotate & 1) ? cpu_surface->w : cpu_surface->h,
                 32, SDL_PIXELFORMAT_ARGB8888);

           if (rotated_surface &&
                 sdl_miyoomini_rotate_to_surface(rotated_surface,
                       cpu_surface, rotate) == 0)
           {
              SDL_SetSurfaceBlendMode(rotated_surface, SDL_BLENDMODE_BLEND);
              SDL_SetSurfaceAlphaMod(rotated_surface, alpha);
              texture_surface = rotated_surface;
           }
           else if (rotated_surface)
           {
        ...
        if (!sdl_miyoomini_upload_overlay_texture(vid, texture_surface)) {
           vid->overlay_enabled = false;
           if (rotated_surface)
              SDL_FreeSurface(rotated_surface);
           SDL_FreeSurface(cpu_surface);
