...
#define SDL_MMIYOOV4_WIDTH_MAX 752
...
#define OSD_TEXT_LINE_LEN_MAX ((uint32_t)(SDL_MMIYOOV4_WIDTH_MAX / (FONT_WIDTH_STRIDE * 2))-1)
...
static void sdl_miyoomini_print_msg(void* data) {
   ...
   if (str_len) {
      uint32_t width = (res_x > 0) ? (uint32_t)res_x : SDL_MIYOOMINI_WIDTH;
      uint32_t osd_line_len = (uint32_t)(width / (FONT_WIDTH_STRIDE * 2)) - 1;
      if (osd_line_len < 1)
         osd_line_len = 1;
